...
The 3D score modes (`3d_lines`, `3d_note_halos`; `is3DVariation()` in `src/core/types.ts`) use a separate pure mapper, `map3DGeometry()` (`src/core/mapper/map3d.ts`), which runs the corresponding 2D mapper (`lines` / `circles`), fits it to the canvas, then lifts each primitive into `RenderedGeometry3D` by attaching a Z coordinate from note time. Z depth is normalized (`effectiveZScale()`: total depth ≈ canvas width × `zScale`/100) so any-length score reads as a proportioned solid. Rendering is owned by `ThreeDRenderer` (`src/renderers/three/`), which implements the `I3DRenderer` interface and is the only module that imports `three`. `src/ui/app.ts` dynamically imports it on first 3D activation, swaps the visible `<canvas>` (2D vs WebGL), and drives it with `ViewportTransform3D` (camera `preset` + `bloom`). The renderer is on-demand (each mutator renders one frame; during playback only the sweeping now-plane updates per tick) and must release GPU resources through `dispose()`. 3D modes export to PNG (`capturePNG()`); SVG/plotter export and the CLI remain 2D-only. See [`plans/2026-07-24-3d-calligraphic-modes.md`](plans/2026-07-24-3d-calligraphic-modes.md).

...
### Added
- Four configurable visual-property options, all defaulting to the previous behavior: a "Length source" selector that makes line/halo length (and 3D depth extent) proportional to velocity instead of duration with a minimum-length floor so quiet notes stay visible; a "Velocity glow" toggle that scales color saturation (all renderers) and 2D canvas shadow glow (capped at 24px) with note velocity; a "Uniform stroke" toggle that draws every note at the base stroke width instead of velocity-scaled thickness; and an "Onset ring flashes" toggle to disable the transient onset rings in the 3D modes during playback. The launch randomizer rolls all four options independently alongside the random visual mode and demo MIDI, syncing the controls to the rolled values. SemVer: **MINOR**.

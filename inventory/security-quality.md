...
for the required-check and data-handling model. Repo-side identity leakage (emails, absolute
paths, private IPs) is covered by `hooks/scripts/check_public_repo_clean.py`, wired as
`check-public-repo-clean` in the root `.pre-commit-config.yaml`.

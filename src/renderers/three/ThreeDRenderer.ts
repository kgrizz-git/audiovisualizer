...
export class ThreeDRenderer implements I3DRenderer {
  ...
  private rebuildContent(): void {
    ...
    if (this.geometry.discs.length > 0) {
      if (this.geometry.config.variation === '3d_note_spheres') this.buildSpheres(this.geometry);
      else this.buildDiscs(this.geometry);
    }
    if (this.geometry.boxes.length > 0) this.buildBoxes(this.geometry);
   ...
   */
  private buildLineSlabs(geometry: RenderedGeometry3D): void {
    if (geometry.segments.length === 0) return;
    this.buildSlabSet(geometry, /* tail */ false);
    this.buildSlabSet(geometry, /* tail */ true);
  }

  private buildSlabSet(geometry: RenderedGeometry3D, tail: boolean): void {
    ...
    const color = new THREE.Color();
    segs.forEach((seg, i) => {
      const startZ = this.worldZ(seg.startZ);
      const endZ = this.worldZ(seg.endZ);
      ...
      const cx = (this.worldX(seg.startX) + this.worldX(seg.endX)) / 2;
      const cy = (this.worldY(seg.startY) + this.worldY(seg.endY)) / 2;
      const cz = (z0 + z1) / 2;
  ...
  private buildDiscs(geometry: RenderedGeometry3D): void {
    ...
    capMaterial.clippingPlanes = [this.revealPlane];
    ...
    bodyMaterial.clippingPlanes = [this.revealPlane];
    ...
    tailMaterial.clippingPlanes = [this.revealPlane];
    ...
    geometry.discs.forEach((disc, i) => {
      ...
      matrix.setPosition(this.worldX(disc.cx), this.worldY(disc.cy), cz);
      ...
      matrix.setPosition(this.worldX(disc.cx), this.worldY(disc.cy), bodyCenterZ);
      ...
      matrix.setPosition(this.worldX(disc.cx), this.worldY(disc.cy), tailCenterZ);
    ...
    this.contentGroup.add(caps, bodies, tails, rings);
    this.revealMaterials.push(capMaterial, bodyMaterial, tailMaterial, ringMaterial);
  }
   ...
   */
  private buildSpheres(geometry: RenderedGeometry3D): void {
    const count = geometry.discs.length;
    ...
    coreMaterial.clippingPlanes = [this.revealPlane];
    ...
    haloMaterial.clippingPlanes = [this.revealPlane];
    ...
    geometry.discs.forEach((disc, i) => {
      ...
      const cx = this.worldX(disc.cx);
      const cy = this.worldY(disc.cy);
      const cz = this.worldZ(disc.cz);
    ...
    this.contentGroup.add(halos, cores);
